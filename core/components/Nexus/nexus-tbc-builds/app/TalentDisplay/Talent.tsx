...
import styles from '../ClassicBuilds.module.sass'
...
const Talent: React.FC<Props> = props => {
  ...
  return (
    <div className={styles.talent} data-has-points={talent.points > 0}>
      ...
      <div className={`${styles.paths} ${pathClasses.map(c => ` ${styles[c]}`)}`}>
        <i /><i /><i /><i />
      </div>

      <div className={styles.talentBackdrop}>
        {!talent.isBlank &&
          <a
            ref={ref}
            href="#"
          ...
          >
            <i />
            <span className={styles.points}>{talent.points}/{talent.maxPoints}</span>
          </a>
        }
      </div>
